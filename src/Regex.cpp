// Includes
#include "Regex.h"
#include <cstdio>								// printfs
#include <cstring>								// memory operations like memcmp() memset()
#include <string>
...
#define		   IsNumberNegative(num1) ((((INT32) (num1)) & 0x80000000) >> 31)	// (num1<0   ) ? 1 : 0
inline INT32 IsNotZero( INT32  num1     ){	return IsNumberNegative(num1 | -num1); }			// (num1!=0  ) ? 1 : 0, Since (-X)|X is 0 for X==0 and negative for every other X.
#define		   IsNotEqual(	   num1,num2)	IsNotZero(num1 ^ num2)						// (num1!=num2)? 1 : 0
#define		   IsEqual(num1,num2)	(IsNotZero(num1 ^ num2)^0x1)					// (num1==num2)? 1 : 0
//#define        IsInRange(i, L, U)  ((((INT32((i) - (L))) | (INT32((U) - (i)))) ^ 0x80000000) >> 31)	// ((i>=L)&&(i<=U) ? 1 : 0
#define        IsInRange(i, L, U)  ((i >= L) && (i <= U))	// ((i>=L)&&(i<=U) ? 1 : 0
// #define        isdigit(               chr)       IsInRange(chr,'0','9')					// ('9'>=chr>='0') ? 1 : 0

/****************************************************************************/
static int atoui_simple(const char* str){			// Read positive integer from string. Like "123" is converted to 123.
	int iResult = 0;
	for (; isdigit(*str); str++)
        iResult = iResult * 10 + (str[0] - '0');
	return iResult;
}
...
static inline int c_achar(Regex* compiledRegexPtr, const char* pat, const char* sam);					 // Handling single char comparison. Like 'b'
static inline int c_any(Regex* compiledRegexPtr, const char* pat, const char* sam);					 // Handlling 'any' comparison. '.'  Note: '.' may consume few bytes (1 char) when working with unicodes, and only 1 byte (= 1 char) for ascii.
static inline int c_extended(Regex* compiledRegexPtr, const char* pat, const char* sam);					 // Handaling special extended abreviations starting with '\\'
static inline int c_group(Regex* compiledRegexPtr, const char* pat, const char* sam);					 // Sub pattern. Grouping characters. Like (the) when searching inside 'I am the master of the realm'
static inline int c_option(Regex* compiledRegexPtr, const char* pat, const char* sam);					 // Selection of one option to match [aA]ce mathces both words: Ace and ace
static inline int c_multi(Regex* compiledRegexPtr, const char* pat, const char* sam, const char* endp);// Multiple occurance of the character. Like A+, A* A{4} A?

// Define pointers to functions that process the special commands. 2 Types
typedef int (*Stand_Func)(Regex* compiledRegexPtr, const char* pat, const char* sam                 );	// Standard functions and characters like () . [] \\ A 7 ....
typedef int (*SuffixFunc)(Regex* compiledRegexPtr, const char* pat, const char* sam, const char* endp);	// Suffix functions for multiple occurences like * + ? {}
...
static const Command* get_cmd_byChar[128];
...
static inline void init_regex_mechanism_private()
{
	if (isInitialized)
	    return;
    const Command* cmd = cmd_tbl, *end = cmd_tbl;
...
static inline const char* findExpressionEnd_uc(const char *p, const char rp){
	...
	for (p++; IsNotZero(*p)&IsNotZero(depth); p++){
		depth += isValidCommand * (IsEqual(*p,lp) - IsEqual(*p,rp));	// Update the depth only if parenthesis is valid. each 'lp' causes +1, rp causes -1
		isValidCommand = IsNotEqual(*p,'\\');			// If current charachter == '\' than the parentheses become invalid
	}
...
static inline const char* findExpressionEnd_UnionSet_uc(const char *p, const char lp, const char rp){
	...
	for (p++; (*p!='\0')&&(depth!=0); p++){
		depth += isValidCommand * (IsEqual(*p,lp) - IsEqual(*p,rp));
		depth -= IsEqual(depth,1)&IsEqual(*p,'|');					// '|' affects only on top level. For example: (XXX(a|B|)DDD) the '|' are not possible alternatives
		isValidCommand = IsNotEqual(*p,'\\');
	}
...
static const char* goToNextPat_uc(const char* cur){   	
    const Command* cmd = get_cmd(*cur);						// Get the command	
...
const char* Regex::Compile(const char *pat)
{
...
// static const Regex* compiledRegexPtr = NULL;	// Pointer to the current regex-used. This line ruins multi-threading capabilities. Feel free to change the system architecture to support mutli-threading
...
static int match(Regex* compiledRegexPtr, const char* pat, const char* sam, const char* endp);

/****************************************************************************/
// Any char comparison is always true
static inline int c_any(Regex* compiledRegexPtr, const char* pat, const char* sam){
	return 1;
}

/****************************************************************************/
// Single char comparison. Match uses one charactr. Wrong returns NO_MATCH.
static inline int c_achar(Regex* compiledRegexPtr, const char* pat, const char* sam){
	return (*pat == *sam) ? 1 : NO_MATCH;
}

/****************************************************************************/
static inline int c_group(Regex* compiledRegexPtr, const char* pat, const char* sam){
	...
	int nCharsMatched;
	while (close[-1]=='|'){
		nCharsMatched = match(compiledRegexPtr, pat+1, sam, close-1);		// +1 and -1 remove the parentheses
		if (nCharsMatched >= 0)
    ...
    return match(compiledRegexPtr, pat+1, sam, close-1);					// Execute the final alternative.
}

/****************************************************************************/
// All possible abbreviations
static inline int c_extended(Regex* compiledRegexPtr, const char* pat, const char* sam){
    ...
    if (*abbr)	return match(compiledRegexPtr, abbr, sam, endOfString(abbr));
    else		return c_achar(compiledRegexPtr, pat,sam);						// Unknown abbreviation. Just assume that it is a character comparison
}

/****************************************************************************/
// Chose one of the options in []. Like [\\-0-9$_#]
static inline  int c_option(Regex* compiledRegexPtr, const char* pat, const char* sam)
{
    ...
    while (pat < close)
    {
        ...
        if (*from == '\\'){ 
			from++;
			pat++;										// Comparison with reserved character. like \\* or \\?
		}
...
static inline int c_multi(Regex* compiledRegexPtr, const char* pat, const char* sam, const char* endp){
    const Command* cmd = get_cmd(*pat);
	...
	if ((min==0)&&(*next_pat)){
		nRestCharsMatched = match(compiledRegexPtr, next_pat, sam, endp);
	...
	nRepitions = 0;
    while (sam < ends){
        nCharsMatched = ((Stand_Func)cmd->f)(compiledRegexPtr, pat, sam);									// Find the pattern for the i'th time.
        if (nCharsMatched < 0){			
...
static int match(Regex* compiledRegexPtr, const char* pat, const char* sam, const char* endp){
    ...
    while (pat < endp)
    {
		...
		if (isSuffix(cmd)){												// 'cmd' is indeed a suffix. like 'z{3,7}'. Activate {3,7} on pattern 'z'.
			int matchedLen = ((SuffixFunc)cmd->f)(compiledRegexPtr, pat, sam, endp);		// Execute the suffix
            ...
            nCharsMatched = ((Stand_Func)cmd->f)(compiledRegexPtr, pat, sam);
            if (nCharsMatched < 0) 
...
const char* Regex::Search(const char* sampleString, int* resLen) {
    ...
    for (; sampleString != endOfSearch; sampleString++){
        *resLen = match(this, pattern, sampleString, endPattern);
	...
	*resLen = match(this, pattern, sampleString, endPattern);					// Try to match pattern to empty string (since we didn't allow it before).
     return (*resLen >= 0) ? sampleString : nullptr;					// Empty sample string was matched to pattern (like a*b*c*) (if==0). Otherwise no match
};
...
static void regex_debug_private_tests()
{
