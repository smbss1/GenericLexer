...
void GeneratePrint(EnodeNode* pRoot);
...
{
    global_source = fopen("src/code.gen.cpp", "w");
    global_header = fopen("include/code.gen.h", "w");
    GenerateTekHeader(global_source, "code.gen");
    GenerateTekHeader(global_header, "code.gen");
    fprintf(global_source, "#include <stdlib.h>\n");
    fprintf(global_source, "#include \"code.gen.h\"\n\n");
}

ENODE_FUNC void EnodeCustomParseCallback(EnodeNode* pRoot, char *strFilename)
{
    GenerateEnodeGraphAsC(global_header, pRoot, false);
}

ENODE_FUNC void EnodeCustomParseStructCallback(EnodeNode* pRoot, char *strFilename)
{
    GenerateEnodeGraphAsC(global_header, pRoot, false);
...
ENODE_FUNC void EnodeCustomParseUnionCallback(EnodeNode* pRoot, char *strFilename)
{
    GenerateEnodeGraphAsC(global_header, pRoot, false);
}

ENODE_FUNC void EnodeCustomParseEnumCallback(EnodeNode* pRoot, char *strFilename)
{
    GenerateEnodeGraphAsC(global_header, pRoot, false);
}
    ...
    if (!first_check_type(fp, pNode, acc_str)) {
        ...
        if (pNode->Declaration.m_pType->TypeUsage.m_pTypeDefinition &&
            ...
            EnodeNode *pFirst_member =
                pNode->Declaration.m_pType->TypeUsage.m_pTypeDefinition->StructDeclaration.m_pFirstMember;
            sprintf(next_acc_str, "%s%s%s", acc_str, pNode->m_strText.c_str(),
                       cond ? "->" : ".");
            ...
            EnodeNode *first_memb =
                pNode->Declaration.m_pType->TypeUsage.m_pStructDeclaration->StructDeclaration.m_pFirstMember;
...
void GeneratePrint(EnodeNode* pRoot)
{
    fprintf(global_header, "void print_%s_%s", pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    fprintf(global_header, "(%s_%s *obj);\n", pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    fprintf(global_source, "void print_%s_%s", pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    fprintf(global_source, "(%s_%s *obj)\n{\n", pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    generate_print_code(global_source, pRoot->StructDeclaration.m_pFirstMember, "obj->");
...
void GenerateSetter(EnodeNode* pRoot)
{
    ...
    fprintf(global_source, "bool init_%s_%s(%s_%s **obj",
             pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str(), pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    generate_procedure(pRoot->StructDeclaration.m_pFirstMember);
    fprintf(global_source, "    *obj = malloc(sizeof(**obj));\n");
    fprintf(global_source, "    if (!*obj)\n        return (false);\n");
    fprintf(global_source, "    my_memset(*obj, 0, sizeof(**obj));\n");
    generate_setter_code(pRoot->StructDeclaration.m_pFirstMember, "obj->");
    ...
    fprintf(global_source, "(%s_%s *obj)\n{\n", pRoot->m_strNamespace.c_str(), pRoot->m_strText.c_str());
    generate_destroyer_code(pRoot->StructDeclaration.m_pFirstMember);
}